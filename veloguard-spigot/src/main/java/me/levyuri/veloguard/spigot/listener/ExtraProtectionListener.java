 ...
 * By disallowing some of these commands, you can decrease the chance that someone will mess up the storage of UUIDs by bypassing VeloGuard.
 */
public class ExtraProtectionListener implements Listener {
    ...
    public void onPlayerCommandPreprocess(PlayerCommandPreprocessEvent e) {
        ...
        // Disable VeloGuard in-game manipulation
        if (messageToLower.contains("veloguard") && BLOCKED_COMMANDS.stream().anyMatch(messageToLower::contains)) {
            e.setCancelled(true);
            player.sendMessage(ChatColor.RED + "You cannot manipulate the VeloGuard from here.");
            logger.warning(String.format("The player %s tried to disable the VeloGuard in-game. (\"%s\")", player.getName(), message));
            return;
        }
    ...
    public void onAsyncPlayerPreLogin(AsyncPlayerPreLoginEvent e) {
        ...
        // If the IP address is null, this can cause an exception and bypass VeloGuard.
